...
### Data loading
```{r}
# gtf <- "/Volumes/jiayiwang/pacbio/data/genome/gencode.v47.chr_patch_hapl_scaff.annotation.sorted.gtf"
# gr <- rtracklayer::import(gtf)
# tx <- gr[mcols(gr)$type == "transcript"]
# tx_df <- as.data.frame(mcols(tx))
# tx_df <- tx_df[, intersect(c("transcript_id", "gene_id", "gene_name"), colnames(tx_df)), drop = FALSE]
# tx_df <- tx_df[!is.na(tx_df$transcript_id) & !is.na(tx_df$gene_id), , drop = FALSE]
# tx_df <- tx_df[!duplicated(tx_df$transcript_id), , drop = FALSE]
# map_gene_id   <- setNames(tx_df$gene_id,   tx_df$transcript_id)
# map_gene_name <- if ("gene_name" %in% names(tx_df)) setNames(tx_df$gene_name, tx_df$transcript_id) else NULL
tx <- fread("data/transcriptome/target_transcripts.csv", header = FALSE)
samples <- c("sampleB_twist_all",
             "sampleA_twist_all",
             "sampleA_all",
             "sampleB_all"
...
sce_dir <- "/Volumes/jiayiwang/loggedfs_david_penton/result/quantify/transcriptome/transcript_sce"
sces <- lapply(samples, \(smp) {
    sce <- readRDS(file.path(sce_dir, paste0(smp, ".rds")))
})
sce <- do.call(cbind, sces)
rd <- DataFrame(tx = rownames(sce))
rd$target <- ifelse(rd$tx %in% tx$V1, "On-Target", "Off-Target")
rowData(sce) <- rd
...
```{r}
dt <- rbindlist(lapply(samples, \(s) {
    x <- sce[,sce$sample==s]
    cnt <- counts(x)  
    data.table(
        transcript = rowData(x)$tx,
        target   = rowData(x)$target,
        count  = rowSums(cnt),
        sample = x$sample[1]
    )
}))
dt[, tech:=ifelse(grepl("twist", sample),"Twist", "WTA")]
dt[, sample_id:=substr(sample, 1, 7)]

dt_wide <- dcast(
  dt, transcript + sample_id + target ~ tech, 
  value.var = "count", 
  fun.aggregate = sum  
)

#dt_wide <- na.omit(dt_wide)
target_colors <- c("Off-Target" = "grey85", "On-Target" = "dodgerblue")
p1 <- ggplot(dt_wide, aes(x = WTA, y = Twist, color = target)) +
  geom_point_rast(size = 1.5, alpha=0.6) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "grey50")  +
...
# ggsave("/Volumes/jiayiwang/twist-comparison/plts//enrichment_transcript.pdf",
#         p1,  width=8, height=3.5)
```
